...
class Philosopher(Child):
    def __init__(self, identifier, fork_left, fork_right, portNear, portFar):
        ...
        self.run = False
        print('[F'+str(self.id) +']-' + self.ip + ': Filósofo '+str(self.id)+' criado.')
    ...
    def dine(self):
        print('[F'+str(self.id) +']-Entrei na funcao comer')
        ...
        clientFar = None
        while self.run:
            print('[F'+str(self.id) +']-Tentando adquirir garfo perto, BLOQUEANDO')
            clientNear = self.acquire_blocking(fork_l, portNear)  # TEM que pegar o garfo esquerdo            
            print('[F'+str(self.id) +']-Tentando adquirir garfo perto, 100 BLOQUEAR')
            ...
            if not disponivel:
                ...
                print('[F'+str(self.id) +']- não consegui, vou devolver o garfo...')
                ...
                print('[F'+str(self.id) +']-Filosofo '+str(self.id)+ ' caiu no ELSE- break')
                break
        if self.run:
            print('[F'+str(self.id) +']-Filosofo ' + str(self.id) + ' vai reportar status e dormir')
            self.dining() # reporta Status e dorme um cadinho
            print('[F'+str(self.id) +']-Filosofo ' + str(self.id) + ' tentando devolver o garfo PERTO')
            if (self.liberaGarfo(clientNear)):
                print('[F'+str(self.id) +']-Garfo devolvido: ', clientNear[0])
            print('[F'+str(self.id) +']-Filosofo ' + str(self.id) + ' tentando devolver o garfo LONGE')
            if (self.liberaGarfo(clientFar)):
                print('[F'+str(self.id) +']-Garfo devolvido: ', clientFar[0])     
    # Bloqueia a execucao até o filosofo adiquirir o garfo
    def acquire_blocking(self, garfoIp, porta):
        while self.run:
            try:
                time.sleep(0.2)
                print('[F'+str(self.id) +']-Criando client para pedir garfo no IP: ', garfoIp, ' pela porta: ', porta)
                client = SocketClient()
                client.connect(garfoIp, porta)
                print('[F'+str(self.id) +']-Pdeindo garfo em no IP: ', garfoIp, ' pela porta: ', porta)
                client.send(pickle.dumps([self.id, 1]))
                data = client.receive()
                if data:
                    response = pickle.loads(data)
                    print('[F'+str(self.id) +']-INFORMACOES vindas de: ', garfoIp, ' pela porta: ', porta)
                    print('[F'+str(self.id) +']-In. dta:\n', response)
                    ...
                    print('[F'+str(self.id) +']-Fim. dta\n')
                    if response[1] == self.id and response[2] == 1 and response[3] == 1:
                        print('[F'+str(self.id) +']-Retornando cliente - SocketClient()')
                        print('[F'+str(self.id) +']-Garfo: ', response[0],'Sucesso: ', response[3])
                        client.close()
                ...
                print('[F'+str(self.id) +']-Except Pass')
                pass
    ...
    def dining(self):
        print('[F'+str(self.id) +']-Filosofo ',str(self.id), ' reportando Status')
        self.report_status(pickle.dumps([self.id, 2]))  # COMENDO - reporta status p/ display
        time.sleep(random.randint(3, 6)) # dorme random
        print('[F'+str(self.id) +']-Filosofo ',str(self.id), ' Fim do sleep, Acordei!')
