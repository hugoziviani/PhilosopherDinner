...
class Garfo(SocketServer, Child):
    def __init__(self, identifier, host, port):
        ...
        if (VERBOSE):
            print('[G' + str(self.identifier) + ']-Garfo '+str(self.identifier)+' criado.')

    def start(self):
        self.run = True
        super(Garfo, self).start() #Chama a handle_connection
        print('[G' + str(self.identifier) + ']-Garfo '+str(self.identifier)+' Up.')
        

    def stop(self):
        self.run = False
        print('[G' + str(self.identifier) + ']-Garfo '+str(self.identifier)+' Down.')
        ...
        while self.run:
            try:
                ...
                if data:
                    ...
                    print('[G' + str(self.identifier) + ']-Filósofo ', request[0],' quer:', request[1], '####')
                    ...
                    if request[1] == 1:
                        if self.sendoUsado:  # Não Disponível
                            ...
                            print ('[G' + str(self.identifier) + ']-Garfo em uso')
                            ...
                            print ('[G' + str(self.identifier) + ']-Garfo disponível, pode pegar. Filósofo: ', request[0], ' - Está usando')
                    ...
                    # Entregar o garfo
                        ...
                        if self.sendoUsado and self.quemEstaUsando == request[0]:
                            ...
                            # success = 1 for success
                            print ('[G' + str(self.identifier) + ']-Garfo devolvido')
                            ...
                            # success = 0 for failure
                            print ('[G' + str(self.identifier) + ']-Garfo não devolvido')
                            connection.send(pickle.dumps([self.identifier, request[0], 0, 0]))
                            break
            except:
                print('[G' + str(self.identifier) + ']-Caiu no except -->', exp)
                break
        connection.close()
        print('[G' + str(self.identifier) + ']-Saiu - fechou conexão')
